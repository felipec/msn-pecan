...
#if defined(LIBPURPLE_NEW_API)
MsnObject*
msn_object_new_from_image(PurpleStoredImage *image, const char *location,
		const char *creator, MsnObjectType type)
{
	...
	img = pecan_buffer_new_memdup ((const gpointer)
		purple_imgstore_get_data (image), purple_imgstore_get_size (image));
